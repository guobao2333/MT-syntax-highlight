...
1. **报告问题**
   - 错误信息：比如索引链接不到语法
   - 过时内容：比如语法高亮已发布但变更日志还没更新
   - 缺失信息：比如某个语法高亮缺少预览图链接
...
2. **创建新 Issue**
   - 请使用适当的模板（已为您提供了 issue 模板）
   - 提供详细信息：问题现象、期望行为、实际行为等
   - 包含复现语法：如果能提供复现语法，将帮助我们更快定位问题
...
- **版本号**：遵循 [语义化版本](https://semver.org)，格式为 `x.y.z`。我们允许您忽略`z`只更新次要版本号。
